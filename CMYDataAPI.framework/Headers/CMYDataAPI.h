 ...
 */
+ (void)startWithConfigOptions:(nonnull CMYInitOptions *)configOptions NS_SWIFT_NAME(start(configOptions:));
 ...
 * @return 返回的单例
 */
+ (CMYDataAPI * _Nullable)sharedInstance;
...
@interface CMYDataAPI (Deprecated)
 ...
 *
 ...
 @return 返回的单例
 */
+ (CMYDataAPI *)sharedInstanceWithConfig:(nonnull CMYInitOptions *)configOptions __attribute__((deprecated("已过时，请使用 + (void)startWithConfigOptions: 方法进行初始化")));
 ...
 * @return 返回的单例
 */
+ (CMYDataAPI *)sharedInstanceWithServerURL:(nullable NSString *)serverURL
 ...
 * @return 返回的单例
 */
+ (CMYDataAPI *)sharedInstanceWithServerURL:(nonnull NSString *)serverURL
